...
void AutoTele::OnLoad() {
	...
	// FIXME: Commenting out bnetBools and gamefilterBools
	...
	new Checkhook(settingsTab, col, (Y += 15),
			&Toggles["Draw Destination"].isEnabled, "Draw Paths");

	new Checkhook(settingsTab, col, (Y += 15),
			&Toggles["Quest Drop Warning"].isEnabled, "Quest Drop Warning");
...
void AutoTele::OnAutomapDraw() {
	...
	if(TPath.GetSize() && Toggles["Draw Path"].isEnabled) {
	...
	if(Toggles["Draw Destination"].isEnabled) {
		for(int i = 0;i<5;i++) {
...
void AutoTele::ManageTele(Vector T) {
	...
	if(Areas[0] == MAP_A2_CANYON_OF_THE_MAGI) {
		...
		if(Toggles["CP to cave"].isEnabled)
...
void AutoTele::PrintText(DWORD Color, char *szText, ...) {
	if(Toggles["Display Messages"].isEnabled) {
		char szBuffer[152] = {0};
		va_list Args;
