...
DrawDirective automapDrawMaphack(true, 5);
...
void Maphack::ReadConfig() {
	/*App.config->ReadInt("Reveal Mode", revealType);
	App.config->ReadInt("Show Monster Resistance", monsterResistanceThreshold);
	App.config->ReadInt("LK Chest Lines", lkLinesColor);
	App.config->ReadInt("Manaburn Monster Color", mbMonColor);

	App.config->ReadKey("Reload Config", "VK_NUMPAD0", reloadConfig);
	App.config->ReadToggle("Show Settings", "VK_NUMPAD8", true, Toggles["Show Settings"]);

	App.config->ReadAssoc("Missile Color", missileColors);
	App.config->ReadAssoc("Monster Color", monsterColors);*/

	// FIXME: Harcoding for now until I figure out how the configs work
	// https://github.com/planqi/slashdiablo-maphack/blob/6226ef321c58270481b7b3d46985909f7898c904/Packaging/BH.cfg
	// https://github.com/planqi/slashdiablo-maphack/blob/6226ef321c58270481b7b3d46985909f7898c904/BH/Modules/Maphack/Maphack.cpp#L50
	revealType = MaphackRevealAct; // 1;
	monsterResistanceThreshold = 99;
	lkLinesColor = 105;
	mbMonColor = 0;
	missileColors["Player"] = 0x97;
	...
	// BH::config->ReadAssoc("Monster Color", MonsterColors); // FIXME: Commented out
...
void Maphack::ResetPatches() {
	//Lighting Patch
	if (Toggles["Force Light Radius"].isEnabled)
		lightingPatch->Install();
	else
		lightingPatch->Remove();

	//Weather Patch
	if (Toggles["Remove Weather"].isEnabled)
		weatherPatch->Install();
	else
		weatherPatch->Remove();

	//Infravision Patch
	if (Toggles["Infravision"].isEnabled)
		infraPatch->Install();
	else
		infraPatch->Remove();
		//GameShake Patch
	if (Toggles["Remove Shake"].isEnabled)
		shakePatch->Install();
	else
		shakePatch->Remove();
	//Monster Health Bar Patch
	if (Toggles["Monster Resistances"].isEnabled)
		monsterNamePatch->Install();
	else
		monsterNamePatch->Remove();

	// patch for cpu-overutilization fix
	if (Toggles["Apply CPU Patch"].isEnabled)
		cpuPatch->Install();
	else
		cpuPatch->Remove();

	if (Toggles["Apply FPS Patch"].isEnabled)
		fpsPatch->Install();
	else
		fpsPatch->Remove();

	if (Toggles["Skip NPC Quest Messages"].isEnabled) {
...
void Maphack::OnLoad() {
	...
	int keyhook_x = 150;
	int col2_x = 250;
	// FIXME: Just commenting for now
	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Auto Reveal"].isEnabled, "Auto Reveal");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Auto Reveal"].hotkey, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Show Monsters"].isEnabled, "Show Monsters");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Show Monsters"].hotkey, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Monster Enchantments"].isEnabled, "  Enchantments");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Monster Enchantments"].hotkey, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Monster Resistances"].isEnabled, "  Resistances");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Monster Resistances"].hotkey, "");
	
	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Show Missiles"].isEnabled, "Show Missiles");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Show Missiles"].hotkey, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Show Chests"].isEnabled, "Show Chests");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Show Chests"].hotkey, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Force Light Radius"].isEnabled, "Light Radius");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Force Light Radius"].hotkey, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Remove Weather"].isEnabled, "Remove Weather");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Remove Weather"].hotkey, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Infravision"].isEnabled, "Infravision");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Infravision"].hotkey, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Remove Shake"].isEnabled, "Remove Shake");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Remove Shake"].hotkey, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Display Level Names"].isEnabled, "Level Names");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Display Level Names"].hotkey, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Apply CPU Patch"].isEnabled, "CPU Patch");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Apply CPU Patch"].hotkey, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Apply FPS Patch"].isEnabled, "FPS Patch (SP Only)");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Apply FPS Patch"].hotkey, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Show Automap On Join"].isEnabled, "Show Automap On Join");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Show Automap On Join"].hotkey, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Skip NPC Quest Messages"].isEnabled, "Skip NPC Quest Messages");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Skip NPC Quest Messages"].hotkey, "");
...
void Maphack::OnLoop() {
	...
	ResetPatches();
	// BH::settingsUI->SetVisible(Toggles["Show Settings"].isEnabled); // FIXME: Commenting out

	// Get the player unit for area information.
	UnitAny* unit = D2CLIENT_GetPlayerUnit();
	if (!unit || !Toggles["Auto Reveal"].isEnabled)
		return;
				...
				if (ItemAttributeMap.find(uInfo.itemCode) != ItemAttributeMap.end()) {
					...
					for (auto &action : actions) {
						if (action.colorOnMap != UNDEFINED_COLOR ||
							...
							// FIXME: Just commenting out
							/*if (action.pingLevel > Item::GetPingLevel()) continue; 
							unit->dwFlags |= UNITFLAG_REVEALED;
							if ((*BH::MiscToggles2)["Item Detailed Notifications"].isEnabled
							  && ((*BH::MiscToggles2)["Item Close Notifications"].isEnabled || (dwFlags & ITEMFLAG_NEW))
...
void Maphack::OnAutomapDraw() {
	...
	if (lastActMaphack != player->pAct){
		lastActMaphack = player->pAct;
		automapDrawMaphack.forceUpdate();
	}
	...
	automapDrawMaphack.draw([=](AsyncDrawBuffer &automapBuffer) -> void {
		POINT MyPos;
		...
		for (Room1* room1 = player->pAct->pRoom1; room1; room1 = room1->pRoomNext) {
			for (UnitAny* unit = room1->pUnitFirst; unit; unit = unit->pListNext) {
				...
				if (unit->dwType == UNIT_MONSTER && IsValidMonster(unit) && Toggles["Show Monsters"].isEnabled) {
					int lineColor = -1;
					...
					string enchantText;
					if (Toggles["Monster Enchantments"].isEnabled) {
				...
				else if (unit->dwType == UNIT_MISSILE && Toggles["Show Missiles"].isEnabled) {
					int color = 255;
					...
					if (ItemAttributeMap.find(uInfo.itemCode) != ItemAttributeMap.end()) {
						...
						for (auto &action : actions) {
							...
							// if (action.pingLevel > Item::GetPingLevel()) continue; // FIXME: Commenting out
				...
				else if (unit->dwType == UNIT_OBJECT && !unit->dwMode /* Not opened */ && Toggles["Show Chests"].isEnabled && IsObjectChest(unit->pObjectData->pTxt)) {
		...
		if (!Toggles["Display Level Names"].isEnabled)
			return;
...
void Maphack::OnGameJoin() {
	ResetRevealed();
	automapLevels.clear();
	*p_D2CLIENT_AutomapOn = Toggles["Show Automap On Join"].isEnabled;
}

// FIXME: Just making prototype
